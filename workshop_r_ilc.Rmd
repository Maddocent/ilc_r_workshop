...
From the smoking dataset we will genrate a new dataframe with average weight, grouped by gender, and grouped by smoking or non smoking. We will use dplyr verse to do so.
...
smoking_weight <- smoking %>% select(genderf, smokef, weight) %>%
  group_by(genderf, smokef) %>% summarise(mean_weight = mean(weight)) %>%
  arrange(desc(weight))
